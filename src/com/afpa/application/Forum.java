...
class Forum implements IForumAbonne, IForumModerateur {
	...
	private ArrayList<Nouvelle> listNouvelle = new ArrayList<>();
	private ArrayList<Abonne> listAbonne = new ArrayList<>();
	...
	/**
	 * Constructeur de la classe forum, assigne la date d'aujourd'hui pour la
	 * création
	 *
	...
	/**
	 * Setter pour le nom du forum, vérifie qu'il n'est pas nul et met en lower
	 * case
	 *
	...
	/**
	 * getter sur la liste des nouvelles qui renvoie l'index d'une nouvelle passée
	 * en paramètre
	 *
	 * @param nouvelle La nouvelle à rechercher dans la liste des nouvelles pour
	 *                 récupérer son index
	 *
	 * @return L'index de position de la nouvelle
	 */
	@Override
	public int getIndex(Nouvelle nouvelle) {
	 ...
	 * Redefinition de toString()
	 *
	 * @return Une présentation du forum avec nom et date de création sous forme
	 *         de String
	 */
	@Override
	...
	/**
	 * Permet d'afficher une liste des messages présents sur le forum. Fait appel
	 * à toStrong() de Nouvelle
	 */
	...
	/**
	 * Créé une nouvelle dans la liste-stockage du forum en réponse à celle
	 * située à Index Le titre sera donc le même.
	 *
	 * @param index  La position de la nouvelle d'origine à laquelle on souhaite
	 *               répondre
	 * @param auteur L'auteur de la réponse pour la création de la
	 *               nouvelle-réponse
	 */
	@Override
	public void repondreNouvelle(int index, Personne auteur) {
		if (listAbonne.contains(auteur)) {
			this.listNouvelle.add(new Nouvelle(this.listNouvelle.get(index).getSujet(), questionWindow(
					String.format("Entrer votre réponse au sujet '%s'.", this.listNouvelle.get(index).getSujet()),
	...
	/**
	 * Permet de bannir un abonné du forum pour cela, lui supprime son attribut
	 * forum et le supprime de la liste-stockage des abonnés du forum
	 *
	 * @param abonne L'abonné à bannir
	 */
	@Override
	public void bannirUnAbonne(Abonne abonne) {
		if (listAbonne.contains(abonne)) {
			System.out.printf("L'abonne %s a ete banni..%n", abonne.toStringShort());
			abonne.setForum(null);
			abonne.setAvertissement(0);
	...
	@Override
	public void avertirUnAbonne(Abonne abonne) {

		if (listAbonne.contains(abonne)) {
			abonne.setAvertissement(abonne.getAvertissement() + 1);
			System.out.printf("L'abonne %s a ete averti. Ca lui fait %d avertissement(s)%n", abonne.toStringShort(),
					abonne.getAvertissement());
			if (abonne.getAvertissement() > 2) {
	 ...
	 *
	 * @return L'index de position de l'abonné dans la liste-stockage des abonnés
	 *         du forum, -1 si échec
	 */
	@Override
